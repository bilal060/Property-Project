...
exports.login = async (req, res) => {
    try {
        ...
        const admin = await Client.findOne({ email: email, removed: false });
        // console.log(admin);
        if (!admin)
            return res.status(400).json({
                success: false,
...
exports.isValidAdminToken = async (req, res, next) => {
    try {
        ...
        const admin = await Client.findOne({ _id: verified.id, removed: false });
        if (!admin)
            return res.status(401).json({
                success: false,
                result: null,
                message: "Client doens't Exist, authorization denied.",
                jwtExpired: true,
            });
        ...
        if (admin.isLoggedIn === false)
            return res.status(401).json({
                success: false,
                result: null,
                message: 'Client is already logout try to login, authorization denied.',
                jwtExpired: true,
            });
        else {
            req.admin = admin;
